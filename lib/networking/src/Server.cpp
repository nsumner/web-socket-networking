...
void HTTPSession::handleRequest()
{
  ...
  if (auto method = request.method();
      method != boost::beast::http::verb::get && method != boost::beast::http::verb::head)
  {
  ...
  auto shouldServeIndex = [](auto target)
  {
    ...
    return target == "/" || (index.size() <= target.size() && target.compare(target.size() - index.size(), npos, index) == 0);
  };
